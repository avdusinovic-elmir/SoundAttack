...
population_array = [150]
elites_array = [0.4]
thresholds = [0.3, 0.2, 0.1, 0.05, 0.01]
mutation_list = [0.5, 0.25]
epsilon_list = [0.9, 0.7, 0.5, 0.25]
...
for i in range(len(dir_list)-1):
    ...
    for target in commands:
        ...
        for popu in population_array:
            for elite_per in elites_array:
                target_text = target.upper()  # Target transcription for the adversarial sample
                population = popu
                elits = int(popu*elite_per)
                epochs = 1000
                for m in mutation_list:
                    for e in epsilon_list:
                        ...
                        sample_name = command + str(i+1) + "_"+ str(population)+ "_"+str(elits)+ "_"+str(m)+"_"+str(e)+ target
                        ...
                        plt.legend()
                        plt.savefig(f"{folder_path}{result_text}_{i+1}_{population}_{elits}_{m}_{e}_{final_epoch}.png")
                        plt.show()
                        ...
                        sf.write(f"{folder_path}{result_text}_{i+1}_{population}_{elits}_{m}_{e}_{final_epoch}.wav", speech_array + noise, sampling_rate)
